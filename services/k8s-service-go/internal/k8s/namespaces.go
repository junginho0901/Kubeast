...
func (s *Service) GetNamespaces(ctx context.Context) ([]map[string]interface{}, error) {
	nsList, err := s.clientsetCtx(ctx).CoreV1().Namespaces().List(ctx, metav1.ListOptions{})
	if err != nil {
...
func (s *Service) DescribeNamespace(ctx context.Context, name string) (map[string]interface{}, error) {
	...
	go func() {
		defer wg.Done()
		ns, nsErr = s.clientsetCtx(ctx).CoreV1().Namespaces().Get(ctx, name, metav1.GetOptions{})
	}()
	go func() {
		defer wg.Done()
		if pods, err := s.clientsetCtx(ctx).CoreV1().Pods(name).List(ctx, metav1.ListOptions{}); err == nil {
			podCount = len(pods.Items)
		}
	}()
	go func() {
		defer wg.Done()
		if svcs, err := s.clientsetCtx(ctx).CoreV1().Services(name).List(ctx, metav1.ListOptions{}); err == nil {
			svcCount = len(svcs.Items)
		}
	}()
	go func() {
		defer wg.Done()
		if deps, err := s.clientsetCtx(ctx).AppsV1().Deployments(name).List(ctx, metav1.ListOptions{}); err == nil {
			depCount = len(deps.Items)
		}
	}()
	go func() {
		defer wg.Done()
		if cms, err := s.clientsetCtx(ctx).CoreV1().ConfigMaps(name).List(ctx, metav1.ListOptions{}); err == nil {
			cmCount = len(cms.Items)
		}
	}()
	go func() {
		defer wg.Done()
		if secrets, err := s.clientsetCtx(ctx).CoreV1().Secrets(name).List(ctx, metav1.ListOptions{}); err == nil {
			secretCount = len(secrets.Items)
		}
	}()
	go func() {
		defer wg.Done()
		quotas, quotasErr = s.clientsetCtx(ctx).CoreV1().ResourceQuotas(name).List(ctx, metav1.ListOptions{})
	}()
	go func() {
		defer wg.Done()
		limits, limitsErr = s.clientsetCtx(ctx).CoreV1().LimitRanges(name).List(ctx, metav1.ListOptions{})
	}()
	go func() {
		defer wg.Done()
		events, eventsErr = s.clientsetCtx(ctx).CoreV1().Events(name).List(ctx, metav1.ListOptions{
...
func (s *Service) CreateNamespace(ctx context.Context, name string, labels map[string]string) (map[string]interface{}, error) {
	...
	created, err := s.clientsetCtx(ctx).CoreV1().Namespaces().Create(ctx, ns, metav1.CreateOptions{})
	if err != nil {
...
func (s *Service) DeleteNamespace(ctx context.Context, name string) error {
	return s.clientsetCtx(ctx).CoreV1().Namespaces().Delete(ctx, name, metav1.DeleteOptions{})
}
...
func (s *Service) GetNamespaceResourceQuotas(ctx context.Context, namespace string) ([]map[string]interface{}, error) {
	quotas, err := s.clientsetCtx(ctx).CoreV1().ResourceQuotas(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
...
func (s *Service) GetNamespaceLimitRanges(ctx context.Context, namespace string) ([]map[string]interface{}, error) {
	limits, err := s.clientsetCtx(ctx).CoreV1().LimitRanges(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
...
func (s *Service) ApplyNamespaceYAML(ctx context.Context, name string, yamlStr string) (map[string]interface{}, error) {
	ns, err := s.clientsetCtx(ctx).CoreV1().Namespaces().Get(ctx, name, metav1.GetOptions{})
	if err != nil {
	...
	updated, err := s.clientsetCtx(ctx).CoreV1().Namespaces().Update(ctx, ns, metav1.UpdateOptions{})
	if err != nil {
