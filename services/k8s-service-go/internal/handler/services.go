...
func (h *Handler) DeleteService(w http.ResponseWriter, r *http.Request) {
	if err := h.requirePermissionForCluster(r, "resource.service.delete"); err != nil {
		h.handleError(w, err)
		return
	}
	ctx := r.Context()
	namespace := chi.URLParam(r, "namespace")
	name := chi.URLParam(r, "name")
	err := h.svc.DeleteService(ctx, namespace, name)
	h.recordAudit(r, "k8s.service.delete", "service", name, namespace, err)
	if err != nil {
		h.handleError(w, err)
		return
	}
