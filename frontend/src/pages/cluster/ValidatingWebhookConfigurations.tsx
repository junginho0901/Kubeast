...
export default function ValidatingWebhookConfigurations() {
  ...
  const handleRefresh = async () => {
    ...
    try {
      const data = await api.getValidatingWebhookConfigurations(true)
      queryClient.removeQueries({ queryKey: ['cluster', 'validatingwebhookconfigurations'] })
      queryClient.setQueryData(['cluster', 'validatingwebhookconfigurations'], data)
    } catch (error) {
      console.error('ValidatingWebhookConfigurations refresh failed:', error)
    }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] gap-4">
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h1 className="text-3xl font-bold text-white">{tr('validatingWebhooks.title', 'Validating Webhook Configurations')}</h1>
          <p className="mt-2 text-slate-400">
            {tr('validatingWebhooks.subtitle', 'Manage validating admission webhook configurations in the cluster.')}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {canCreate && (
            ...
            >
              ...
              {tr('validatingWebhooks.create', 'Create')}
            </button>
          )}
          <button
            type="button"
            ...
            title={tr('validatingWebhooks.refreshTitle', 'Force refresh')}
            className="btn btn-primary flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <RefreshCw className={`w-4 h-4 ${isRefreshing ? 'animate-spin' : ''}`} />
            {tr('validatingWebhooks.refresh', 'Refresh')}
          </button>
        </div>
      </div>
      ...
      <div className="shrink-0">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder={tr('validatingWebhooks.searchPlaceholder', 'Search by name...')}
            value={searchQuery}
      ...
      <div className="grid grid-cols-2 gap-3 shrink-0">
        <div className="rounded-lg border border-slate-700 bg-slate-900/50 px-4 py-3">
          <p className="text-[11px] sm:text-xs leading-4 whitespace-nowrap text-slate-400">{tr('validatingWebhooks.stats.total', 'Total Configurations')}</p>
          <p className="text-lg text-white font-semibold mt-1">{summary.total}</p>
        </div>
        <div className="rounded-lg border border-slate-700 bg-slate-900/50 px-4 py-3">
          <p className="text-[11px] sm:text-xs leading-4 whitespace-nowrap text-slate-400">{tr('validatingWebhooks.stats.totalWebhooks', 'Total Webhooks')}</p>
      ...
      {searchQuery && (
        <p className="text-sm text-slate-400 shrink-0">
          {tr('validatingWebhooks.matchCount', '{{count}} configuration{{suffix}} match.', {
              ...
              <tr>
                <th className="text-left py-3 px-4 w-[400px] cursor-pointer" onClick={() => handleSort('name')}>
                  <span className="inline-flex items-center gap-1">
                    {tr('validatingWebhooks.table.name', 'Name')}{renderSortIcon('name')}
                  </span>
                </th>
                <th className="text-left py-3 px-4 w-[120px] cursor-pointer" onClick={() => handleSort('webhooksCount')}>
                  <span className="inline-flex items-center gap-1">
                    {tr('validatingWebhooks.table.webhooks', 'Webhooks')}{renderSortIcon('webhooksCount')}
                  </span>
                </th>
                <th className="text-left py-3 px-4 w-[100px] cursor-pointer" onClick={() => handleSort('age')}>
                  <span className="inline-flex items-center gap-1">
                    {tr('validatingWebhooks.table.age', 'Age')}{renderSortIcon('age')}
                  </span>
                </th>
              </tr>
            </thead>
              ...
              {sorted.length === 0 && !isLoading && (
                <tr>
                  <td colSpan={3} className="py-6 px-4 text-center text-slate-400">
                    {tr('validatingWebhooks.noResults', 'No validating webhook configurations found.')}
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {createDialogOpen && (
        <ResourceYamlCreateDialog
          title={tr('validatingWebhooks.createTitle', 'Create Validating Webhook Configuration from YAML')}
