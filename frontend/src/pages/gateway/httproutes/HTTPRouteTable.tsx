// HTTPRoutes 목록 테이블 + sort 헤더 + pagination
//
// frontend/src/pages/gateway/HTTPRoutes.tsx 의 table JSX (sort header + tbody +
...
interface Props {
  pagedHTTPRoutes: HTTPRouteInfo[]
  sortedHTTPRoutesLength: number
  isLoading: boolean
...
export function HTTPRouteTable({
  pagedHTTPRoutes,
  sortedHTTPRoutesLength,
  isLoading,
  showNamespaceColumn,
  sortKey,
  setSortKey,
      ...
      <div ref={tableBodyRef} className="overflow-x-auto flex-1 min-h-0">
        <table className="w-full text-sm min-w-[1120px] table-fixed">
          ...
          <tbody className="divide-y divide-slate-700">
            ...
            <AdaptiveTableFillerRows count={rowsPerPage - pagedHTTPRoutes.length} columnCount={7 + (showNamespaceColumn ? 1 : 0)} />
        </table>
      </div>

      {sortedHTTPRoutesLength > 0 && (
        <div className="flex items-center justify-between px-4 py-3 border-t border-slate-700 shrink-0">
          <div className="text-xs text-slate-400">
            {tr('common.paginationRange', 'Showing {{start}}-{{end}} of {{total}}', {
              start: (currentPage - 1) * rowsPerPage + 1,
              end: Math.min(currentPage * rowsPerPage, sortedHTTPRoutesLength),
              total: sortedHTTPRoutesLength,
            })}
          </div>
