...
function formatHostnames(item: GRPCRouteInfo): string {
  const list = Array.isArray(item.hostnames) ? item.hostnames : []
  if (list.length === 0) return '*'
  return list.map((h) => h || '*').join(', ')
}

function normalizeWatchGRPCRouteObject(obj: any): GRPCRouteInfo {
  if (
...
function applyGRPCRouteWatchEvent(
...
function grpcRouteToRawJson(item: GRPCRouteInfo): Record<string, unknown> {
  return {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <div className="xl:col-span-2">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder={tr('grpcRoutesPage.searchPlaceholder', 'Search GRPCRoutes by name...')}
              value={searchQuery}
        ...
        <div className="relative" ref={namespaceDropdownRef}>
          ...
          >
            <span className="text-sm font-medium">
              {selectedNamespace === 'all' ? tr('grpcRoutesPage.allNamespaces', 'All namespaces') : selectedNamespace}
            </span>
          ...
          {isNamespaceDropdownOpen && (
            <div className="absolute top-full left-0 mt-2 w-full bg-slate-700 border border-slate-600 rounded-lg shadow-xl z-[100] max-h-[240px] overflow-y-auto">
              ...
              >
                ...
                <span className={selectedNamespace === 'all' ? 'font-medium' : ''}>{tr('grpcRoutesPage.allNamespaces', 'All namespaces')}</span>
              </button>
              {(namespaces || []).map((ns) => (
                <button
                  key={ns.name}
      ...
      <div ref={tableContainerRef} className="card">
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[1120px] table-fixed">
            ...
            <tbody className="divide-y divide-slate-700">
              ...
              {sortedGRPCRoutes.length === 0 && !isLoading && (
                <tr>
        ...
        {sortedGRPCRoutes.length > 0 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-slate-700">
            <div className="text-xs text-slate-400">
              {tr('common.paginationRange', 'Showing {{start}}-{{end}} of {{total}}', {
                start: (currentPage - 1) * rowsPerPage + 1,
                end: Math.min(currentPage * rowsPerPage, sortedGRPCRoutes.length),
