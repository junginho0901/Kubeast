...
export default function RoleBindings() {
  ...
  const aiSnapshot = useMemo(() => {
    ...
    const total = roleBindings.length
    return {
      source: 'base' as const,
      summary: `${nsLabel} RoleBinding ${total}개`,
      data: {
        ...
        ...summarizeList(pagedItems as unknown as Record<string, unknown>[], {
          ...
          topN: rowsPerPage,
          pickFields: ['name', 'namespace', 'role_ref_kind', 'role_ref_name', 'subjects_count'],
          linkBuilder: (r) => {
            const rb = r as unknown as RoleBindingInfo
            return buildResourceLink('RoleBinding', rb.namespace, rb.name)
          },
        }),
      },
    }
  }, [roleBindings, pagedItems, sortedItems.length, currentPage, rowsPerPage, selectedNamespace, searchQuery])
