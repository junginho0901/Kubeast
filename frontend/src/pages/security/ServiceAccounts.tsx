...
import { api, type ServiceAccountInfo } from '@/services/api'
...
function normalizeWatchServiceAccountObject(obj: any): ServiceAccountInfo {
  if (
    typeof obj?.name === 'string' &&
    typeof obj?.namespace === 'string' &&
    typeof obj?.secrets === 'number'
  ) {
    return obj as ServiceAccountInfo
  }

  const metadata = obj?.metadata ?? {}
  const secrets = Array.isArray(obj?.secrets) ? obj.secrets : []
...
function applyServiceAccountWatchEvent(
  prev: ServiceAccountInfo[] | undefined,
  ...
  const normalized = normalizeWatchServiceAccountObject(obj)
...
export default function ServiceAccounts() {
  ...
  const { data: serviceAccounts, isLoading } = useQuery({
    queryKey: ['security', 'serviceaccounts', selectedNamespace],
    queryFn: () =>
      selectedNamespace === 'all'
        ? api.getAllServiceAccounts(false)
        : api.getServiceAccounts(selectedNamespace, false),
  })
  ...
  useKubeWatchList({
    ...
    applyEvent: (prev, event) =>
      applyServiceAccountWatchEvent(prev as ServiceAccountInfo[] | undefined, event),
    onEvent: (event) => {
  ...
  const filteredItems = useMemo(() => {
    if (!Array.isArray(serviceAccounts)) return [] as ServiceAccountInfo[]
    if (!searchQuery.trim()) return serviceAccounts
  ...
  const sortedItems = useMemo(() => {
    ...
    const getValue = (sa: ServiceAccountInfo): string | number => {
      switch (sortKey) {
        case 'name': return sa.name
        case 'namespace': return sa.namespace
        case 'secrets': return sa.secrets
        case 'age': return parseAgeSeconds(sa.created_at)
        default: return ''
      }
    }
    list.sort((a, b) => {
  ...
  const handleRefresh = async () => {
    ...
    try {
      const data = selectedNamespace === 'all'
        ? await api.getAllServiceAccounts(true)
        : await api.getServiceAccounts(selectedNamespace, true)
      queryClient.removeQueries({ queryKey: ['security', 'serviceaccounts', selectedNamespace] })
      queryClient.setQueryData(['security', 'serviceaccounts', selectedNamespace], data)
    } catch (error) {
      console.error('ServiceAccounts refresh failed:', error)
    }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] gap-4">
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h1 className="text-3xl font-bold text-white">{tr('serviceAccountsPage.title', 'Service Accounts')}</h1>
          <p className="mt-2 text-slate-400">{tr('serviceAccountsPage.subtitle', 'Manage ServiceAccounts across namespaces.')}</p>
        </div>
        <div className="flex items-center gap-2">
          {canCreate && (
            <button type="button" onClick={() => setCreateDialogOpen(true)} className="btn btn-primary flex items-center gap-2">
              <Plus className="w-4 h-4" />
              {tr('serviceAccountsPage.create', 'Create ServiceAccount')}
            </button>
          )}
          <button type="button" onClick={handleRefresh} disabled={isRefreshing} title={tr('serviceAccountsPage.refreshTitle', 'Force refresh')} className="btn btn-primary flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
            <RefreshCw className={`w-4 h-4 ${isRefreshing ? 'animate-spin' : ''}`} />
            {tr('serviceAccountsPage.refresh', 'Refresh')}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-3 shrink-0">
        <div className="xl:col-span-2">
          <div className="relative">
            ...
            <input type="text" placeholder={tr('serviceAccountsPage.searchPlaceholder', 'Search by name or namespace...')} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="h-12 w-full pl-10 pr-4 bg-slate-700 border border-slate-600 rounded-lg text-sm text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent" />
          </div>
        </div>
        <div className="relative" ref={namespaceDropdownRef}>
          <button type="button" onClick={() => setIsNamespaceDropdownOpen((v) => !v)} className="h-12 w-full px-3 bg-slate-700 border border-slate-600 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent flex items-center justify-between gap-2">
            <span className="text-sm font-medium">{selectedNamespace === 'all' ? tr('serviceAccountsPage.allNamespaces', 'All namespaces') : selectedNamespace}</span>
          ...
          {isNamespaceDropdownOpen && (
            <div className="absolute top-full left-0 mt-2 w-full bg-slate-700 border border-slate-600 rounded-lg shadow-xl z-[100] max-h-[240px] overflow-y-auto">
              <button type="button" onClick={() => { setSelectedNamespace('all'); setIsNamespaceDropdownOpen(false) }} className="w-full px-4 py-2.5 text-left text-sm text-white hover:bg-slate-600 transition-colors flex items-center gap-2 first:rounded-t-lg">
                ...
                <span className={selectedNamespace === 'all' ? 'font-medium' : ''}>{tr('serviceAccountsPage.allNamespaces', 'All namespaces')}</span>
              </button>
              {(namespaces || []).map((ns) => (
      ...
      {searchQuery && (
        <p className="text-sm text-slate-400 shrink-0">
          {tr('serviceAccountsPage.matchCount', '{{count}} result(s) match.', { count: filteredItems.length })}
        </p>
      )}
        ...
        <div className="overflow-x-auto flex-1 min-h-0">
          <table className="w-full text-sm min-w-[800px] table-fixed">
            <thead className="text-slate-400">
              <tr>
                ...
                <th className="text-left py-3 px-4 cursor-pointer" onClick={() => handleSort('name')}>
                  <span className="inline-flex items-center gap-1">{tr('serviceAccountsPage.table.name', 'Name')}{renderSortIcon('name')}</span>
                </th>
            ...
            <tbody className="divide-y divide-slate-700">
              {pagedItems.map((sa) => (
                <tr key={`${sa.namespace}/${sa.name}`} className="text-slate-200 hover:bg-slate-800/60 cursor-pointer" onClick={() => openDetail({ kind: 'ServiceAccount', name: sa.name, namespace: sa.namespace })}>
      ...
      {createDialogOpen && (
        <ResourceYamlCreateDialog
          title={tr('serviceAccountsPage.createTitle', 'Create ServiceAccount from YAML')}
