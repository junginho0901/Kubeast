...
type SortKey = null | 'name' | 'namespace' | 'rules' | 'age'
...
function normalizeWatchRoleObject(obj: any): RoleInfo {
  if (
    typeof obj?.name === 'string' &&
    typeof obj?.namespace === 'string' &&
    typeof obj?.rules_count === 'number'
  ) {
    return obj as RoleInfo
  }
...
function applyRoleWatchEvent(
  prev: RoleInfo[] | undefined,
  ...
  const normalized = normalizeWatchRoleObject(obj)
...
export default function Roles() {
  ...
  const { data: roles, isLoading } = useQuery({
    queryKey: ['security', 'roles', selectedNamespace],
    queryFn: () =>
  ...
  useKubeWatchList({
    ...
    path:
      selectedNamespace === 'all'
        ? '/api/v1/roles'
        : `/api/v1/namespaces/${selectedNamespace}/roles`,
    query: 'watch=1',
    applyEvent: (prev, event) => applyRoleWatchEvent(prev as RoleInfo[] | undefined, event),
    onEvent: (event) => {
  ...
  const filteredItems = useMemo(() => {
    if (!Array.isArray(roles)) return [] as RoleInfo[]
    if (!searchQuery.trim()) return roles
    const q = searchQuery.toLowerCase()
    return roles.filter(
      (role) =>
        role.name.toLowerCase().includes(q) ||
        role.namespace.toLowerCase().includes(q),
    )
  }, [roles, searchQuery])
  ...
  const summary = useMemo(() => {
    ...
    let totalRules = 0
    for (const role of filteredItems) totalRules += role.rules_count
    return { total, totalRules }
  }, [filteredItems])

  const summaryCards = useMemo<SummaryCard[]>(
    () => [
      [tr('rolesPage.stats.total', 'Total'), summary.total, 'border-slate-700 bg-slate-900/50', 'text-slate-400'],
      [tr('rolesPage.stats.totalRules', 'Total Rules'), summary.totalRules, 'border-cyan-700/40 bg-cyan-900/10', 'text-cyan-300'],
    ],
  ...
  const sortedItems = useMemo(() => {
    ...
    const getValue = (role: RoleInfo): string | number => {
      switch (sortKey) {
        case 'name': return role.name
        case 'namespace': return role.namespace
        case 'rules': return role.rules_count
        case 'age': return parseAgeSeconds(role.created_at)
        default: return ''
      }
    }
    list.sort((a, b) => {
    ...
    return {
      ...
      data: {
        ...
        ...summarizeList(pagedItems as unknown as Record<string, unknown>[], {
          ...
          topN: rowsPerPage,
          pickFields: ['name', 'namespace', 'rules_count'],
          linkBuilder: (r) => {
            const ri = r as unknown as RoleInfo
            return buildResourceLink('Role', ri.namespace, ri.name)
          },
        }),
      },
    }
  }, [roles, pagedItems, sortedItems.length, currentPage, rowsPerPage, selectedNamespace, searchQuery])
  ...
  const handleRefresh = async () => {
    ...
    try {
      const data = selectedNamespace === 'all'
        ? await api.getAllRoles(true)
        : await api.getRoles(selectedNamespace, true)
      queryClient.removeQueries({ queryKey: ['security', 'roles', selectedNamespace] })
      queryClient.setQueryData(['security', 'roles', selectedNamespace], data)
    } catch (error) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] gap-4">
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h1 className="text-3xl font-bold text-white">{tr('rolesPage.title', 'Roles')}</h1>
          <p className="mt-2 text-slate-400">{tr('rolesPage.subtitle', 'Manage Roles across namespaces.')}</p>
        </div>
        <div className="flex items-center gap-2">
          {canCreate && (
            <button type="button" onClick={() => setCreateDialogOpen(true)} className="btn btn-primary flex items-center gap-2">
              <Plus className="w-4 h-4" />
              {tr('rolesPage.create', 'Create Role')}
            </button>
          )}
          <button type="button" onClick={handleRefresh} disabled={isRefreshing} title={tr('rolesPage.refreshTitle', 'Force refresh')} className="btn btn-primary flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
            <RefreshCw className={`w-4 h-4 ${isRefreshing ? 'animate-spin' : ''}`} />
            {tr('rolesPage.refresh', 'Refresh')}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-3 shrink-0">
        <div className="xl:col-span-2">
          <div className="relative">
            ...
            <input type="text" placeholder={tr('rolesPage.searchPlaceholder', 'Search by name or namespace...')} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="h-12 w-full pl-10 pr-4 bg-slate-700 border border-slate-600 rounded-lg text-sm text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent" />
          </div>
        </div>
        <div className="relative" ref={namespaceDropdownRef}>
          <button type="button" onClick={() => setIsNamespaceDropdownOpen((v) => !v)} className="h-12 w-full px-3 bg-slate-700 border border-slate-600 rounded-lg text-sm text-white focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent flex items-center justify-between gap-2">
            <span className="text-sm font-medium">{selectedNamespace === 'all' ? tr('rolesPage.allNamespaces', 'All namespaces') : selectedNamespace}</span>
          ...
          {isNamespaceDropdownOpen && (
            <div className="absolute top-full left-0 mt-2 w-full bg-slate-700 border border-slate-600 rounded-lg shadow-xl z-[100] max-h-[240px] overflow-y-auto">
              <button type="button" onClick={() => { setSelectedNamespace('all'); setIsNamespaceDropdownOpen(false) }} className="w-full px-4 py-2.5 text-left text-sm text-white hover:bg-slate-600 transition-colors flex items-center gap-2 first:rounded-t-lg">
                ...
                <span className={selectedNamespace === 'all' ? 'font-medium' : ''}>{tr('rolesPage.allNamespaces', 'All namespaces')}</span>
              </button>
              {(namespaces || []).map((ns) => (
      ...
      {searchQuery && (
        <p className="text-sm text-slate-400 shrink-0">
          {tr('rolesPage.matchCount', '{{count}} result(s) match.', { count: filteredItems.length })}
        </p>
      )}
            ...
            <thead ref={theadRef} className="text-slate-400">
              <tr>
                ...
                <th className="text-left py-3 px-4 cursor-pointer" onClick={() => handleSort('name')}>
                  <span className="inline-flex items-center gap-1">{tr('rolesPage.table.name', 'Name')}{renderSortIcon('name')}</span>
                </th>
      ...
      {createDialogOpen && (
        <ResourceYamlCreateDialog
          title={tr('rolesPage.createTitle', 'Create Role from YAML')}
